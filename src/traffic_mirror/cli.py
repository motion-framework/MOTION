...
from traffic_mirror.config.settings import AppSettings, SettingsError, load_settings
from traffic_mirror.domain.geography import DEFAULT_CENTER_RADIUS_METERS, BoundingBox

if TYPE_CHECKING:
    from traffic_mirror.application.map_provisioning import (
        MapProvisioningService,
        ProvisioningResult,
    )
    from traffic_mirror.domain.maps import MapProfile
    from traffic_mirror.ports.traffic import TrafficDataProvider

LOGGER = logging.getLogger("traffic_mirror")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="traffic-mirror",
        description="MOTION traffic analysis and CARLA traffic-mirroring tools.",
    )
    parser.add_argument(
        "--log-level",
...
def _map_provisioning_service(settings: AppSettings) -> MapProvisioningService:
    ...
    from traffic_mirror.infrastructure.osm.downloader import OsmDownloader
...
def _handle_scan_degenerate(args: argparse.Namespace, _settings: AppSettings) -> int:
    from traffic_mirror.infrastructure.maps.geometry import scan_degenerate_geometries
...
def _handle_scan_overflow(args: argparse.Namespace, _settings: AppSettings) -> int:
    from traffic_mirror.infrastructure.maps.geometry import scan_object_overflows
...
def _handle_patch_zero(args: argparse.Namespace, _settings: AppSettings) -> int:
    from traffic_mirror.infrastructure.maps.geometry import patch_zero_length_geometries
...
def _handle_patch_overflow(args: argparse.Namespace, _settings: AppSettings) -> int:
    from traffic_mirror.infrastructure.maps.geometry import patch_object_overflows
...
def _handle_verify_archive(args: argparse.Namespace, _settings: AppSettings) -> int:
    from traffic_mirror.infrastructure.here.archive import verify_archive
...
def _build_traffic_provider(
    *,
    ...
    if offline:
        from traffic_mirror.infrastructure.offline import build_offline_provider
...
def _run_uc01(
    *,
    ...
    from traffic_mirror.application.use_cases.uc01_real_time_traffic_mirroring.reporting import (
    ...
    from traffic_mirror.infrastructure.carla.adapter import CarlaTrafficSimulator
...
def _handle_model_train(args: argparse.Namespace, settings: AppSettings) -> int:
    from traffic_mirror.prediction.training import train_model_file
...
def _handle_model_infer(args: argparse.Namespace, settings: AppSettings) -> int:
    from traffic_mirror.infrastructure.carla.behavior_monitor import run_behavior_monitor
...
def _handle_telemetry_collect(args: argparse.Namespace, settings: AppSettings) -> int:
    from traffic_mirror.infrastructure.carla.telemetry import collect_telemetry
...
def _handle_speed_units(_args: argparse.Namespace, settings: AppSettings) -> int:
    from traffic_mirror.infrastructure.carla.diagnostics import run_speed_unit_diagnostic

    return run_speed_unit_diagnostic(settings)


def _handle_calibration(args: argparse.Namespace, settings: AppSettings) -> int:
    from traffic_mirror.infrastructure.carla.calibration import run_calibration
