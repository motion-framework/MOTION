...
|---|---|---|---|---|---|---|
| UC-01 | Real-Time Traffic Mirroring | Automatic system; synchronize observed traffic with a CARLA traffic twin. | `traffic_mirror.application.use_cases.uc01_real_time_traffic_mirroring` owns coverage, filtering and the tick loop; `traffic_mirror.domain` owns population and speed policies; `traffic_mirror.infrastructure.here` supplies flow/incidents; `traffic_mirror.infrastructure.carla` owns world lifecycle, population and commands; `traffic-mirror mirror` composes the workflow. | Device registry/feed, HERE Traffic, OSM/OpenDRIVE, CARLA/TrafficManager. | The governance dashboard is not evidenced. Current device readings are synthesized rather than ingested from a production field-device adapter. | **IMPLEMENTED**. |
...
| UC-04 | Multivariate Traffic Prediction | Automatic system; forecast congestion, speed, traffic shape, and anomalies from heterogeneous features. | `traffic_mirror.prediction` preserves a different legacy experiment: per-vehicle incident risk from speed, controls and rain. It does not create the UC-04 15-minute-to-24-hour network traffic forecasts. | Field/temporal/weather/HERE/history data, graph alignment, training, evaluation and persistence. | Feature matrix, history windows, forecast schema, spatial granularity, baselines, metrics, missing-data policy, and graph model. | **NOT IMPLEMENTED / RESEARCH DIRECTION**. |
...
`traffic_mirror.prediction` estimates the legacy per-vehicle
